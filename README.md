...
Jetson 웹 콘솔 실시간 모니터링 화면
![Jetson webconsole_liveview](assets/jetson_web_console_liveview_screenshot.png)
🟩 라이브 센서 뷰와 감지 결과: 이 영역은 EyePatch 센서에서 전달된 ‘마지막’ 프레임을 감지된 객체 주석과 함께 표시합니다. 실제 센서 캡처 시점부터 웹 뷰에 도착하기까지 5–10프레임 정도의 지연이 발생할 수 있으므로, 화면에 보이는 내용이 센서 앞의 실제 실시간 상황을 그대로 반영하지 않을 수 있습니다

🟦 라이브 뷰에서 감지 이벤트를 표시하는 옵션입니다. ‘n 프레임 되돌아보기’는 특정 프레임에서 객체 감지가 끊긴 경우에도 동일한 객체 ID로 연결하기 위해, 감지 이벤트 표시가 과거 n프레임까지 히스토리를 조회하도록 설정합니다

🟨 라이브 뷰 캔버스에서 특정 존(zone)이나 카운트 라인을 표시하기 위한 컨트롤입니다. 존 마킹은 저장되며, 세션이 바뀌어도 유지되고 일일 보고서에도 사용됩니다. 감지 세션이 실행 중일 때 존 마킹을 변경하면, 일일 보고서에는 마지막으로 설정된 존 마킹만 적용됩니다

존 마킹 유형: include — 존 안으로 들어오는 사람을 카운트합니다.
exclude — 사람이 존에 들어오면 경고 메시지를 보냅니다.
등록 기준: 바운딩 박스의 어떤 부분이 존 안에 포함될 때 진입으로 기록할지를 설정합니다
존 안에서 최소 몇 프레임 머물러야 하는지: 객체가 존 안에 일정 시간(프레임 수) 머물렀을 때만 카운트 이벤트로 등록되도록 하는 설정입니다. 이는 아주 잠깐만 존에 스치듯 들어오는 경우나 잘못된 감지(false detection)가 카운트되는 것을 방지합니다.

Jetson 웹 콘솔의 리포트 다운로드 기능
![Jetson webconsole_downloadreports](assets/jetson_web_console_reportsdownload_screenshot.png)
🟦 일일 보고서 다운로드 링크: 하루 동안 생성된 보고서는 감지 이벤트가 여러 개의 개별 디렉터리에 저장되기 때문에 여러 파일로 나누어집니다. 파일 이름에는 해당 날짜와 그 날짜의 시간 구간이 포함됩니다.

🟩 저장된 모든 일일 보고서를 Jetson 장치에서 삭제합니다. 삭제 후에는 복구할 수 없습니다
